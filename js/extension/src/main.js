...
app.initializers.add('squeevee-squeevents', () => {
  if (!squeevents)
    return;

  if (squeevents.enable_log)
    console.log({ squeevents_message: 'Starting Squeevents' });
  ...
  try
  {
    ...
    if (!_data) {
      if (squeevents.enable_log)
        console.log({ squeevents_message: 'No data could be found. Terminating Squeevents.'})
      return;
    }

    if (!_data.squeevents.length) {
      if (squeevents.enable_log)
        console.log({ squeevents_message: 'No Squeevents were registered. Terminating Squeevents.'})
      return;
    }
